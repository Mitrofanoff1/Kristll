...
import { Coffee, Music, Eye, MoveVertical, Sparkles, ChevronLeft, ChevronRight } from 'lucide-react';
...
const Block3 = () => {
  ...
  // Авто-слайдер (можно оставить или убрать, если хочешь только ручной)
  useEffect(() => {
    ...
    <section className="py-12 md:py-24 bg-[#FAFAFA] overflow-hidden">
      <div className="container mx-auto px-4">
        ...
        <div className="max-w-4xl mb-10 lg:mb-20 text-center lg:text-left">
          ...
          {/* Текст скрыт на мобильных, виден на десктопе */}
        ...
        <div className="flex flex-col lg:flex-row gap-10 lg:gap-20">
          ...
          <div className="w-full lg:w-1/2">
            ...
            <div className="lg:hidden relative w-full aspect-[4/5] rounded-[40px] overflow-hidden shadow-xl border-4 border-white">
              {photos.map((photo, index) => (
                <div
                  key={photo.id}
                  className={`absolute inset-0 transition-opacity duration-700 flex items-center justify-center bg-gray-100 ${
                    index === currentImg ? 'opacity-100' : 'opacity-0'
                  }`}
                >
                  <p className="text-gray-400 font-bold uppercase tracking-widest text-center px-10 text-xs">
                    {photo.label}
                  </p>
                </div>
              ))}
              ...
              <button 
                onClick={prevSlide}
                className="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 bg-white/90 backdrop-blur-sm rounded-full flex items-center justify-center shadow-md z-10 active:scale-90"
              >
                <ChevronLeft className="w-6 h-6 text-black" />
              </button>
              <button 
                onClick={nextSlide}
                className="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 bg-white/90 backdrop-blur-sm rounded-full flex items-center justify-center shadow-md z-10 active:scale-90"
              >
            ...
            <div className="hidden lg:grid grid-cols-2 gap-4 h-[700px]">
                ...
                <div className="grid grid-rows-2 gap-4">
                    <div className="bg-gray-100 rounded-[40px] flex items-center justify-center border-2 border-dashed border-gray-200">
                        <span className="text-gray-400 font-bold uppercase text-xs">Напитки</span>
                    </div>
          ...
          <div className="w-full lg:w-1/2 flex flex-col">
            <h3 className="text-sm md:text-base font-black uppercase mb-8 tracking-[0.2em] text-gray-400 text-center lg:text-left">
              Сервис и удобство
            </h3>
            ...
            {/* ФИНАЛЬНЫЙ ТЕКСТ И КНОПКА (Оформлено красиво без плашки) */}
            <div className="mt-12 lg:mt-16 text-center lg:text-left border-t border-gray-200 pt-10">
                ...
                <button className="w-full md:w-auto bg-black hover:bg-zinc-800 text-white text-base md:text-lg font-black py-5 px-16 rounded-full transition-all shadow-xl active:scale-95 uppercase tracking-widest">
                    Записаться
                </button>
            </div>
          </div>
